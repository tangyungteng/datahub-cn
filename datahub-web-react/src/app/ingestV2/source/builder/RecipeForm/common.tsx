...
// Common connection field labels are duplicated as plain strings across dozens of connector
// config files. Translate them at the render layer, falling back to the original label for
// connector-specific fields that have no translation.
const FIELD_LABEL_I18N_KEYS: Record<string, string> = {
    'Host and Port': 'ingestion.host_port',
    Username: 'ingestion.username',
    Password: 'ingestion.password',
    Database: 'ingestion.database',
    'Allow Patterns': 'ingestion.allow_patterns',
    'Deny Patterns': 'ingestion.deny_patterns',
    'Include Tables': 'ingestion.include_tables',
    'Include Views': 'ingestion.include_views',
    'Enable Table Profiling': 'ingestion.enable_table_profiling',
    'Enable Column Profiling': 'ingestion.enable_column_profiling',
    'Enable Stateful Ingestion': 'ingestion.enable_stateful_ingestion',
};

const BUTTON_LABEL_I18N_KEYS: Record<string, string> = {
    'Add pattern': 'ingestion.add_pattern',
};

const SECTION_I18N_KEYS: Record<string, string> = {
    Schemas: 'ingestion.schemas',
    Tables: 'ingestion.tables',
    Views: 'ingestion.views',
};

export function translateLabel(label: any, t: (key: string) => string): any {
    if (typeof label === 'string' && FIELD_LABEL_I18N_KEYS[label]) {
        return t(FIELD_LABEL_I18N_KEYS[label]);
    }
    return label;
}

export function translateButtonLabel(buttonLabel: any, t: (key: string) => string): any {
    if (typeof buttonLabel === 'string' && BUTTON_LABEL_I18N_KEYS[buttonLabel]) {
        return t(BUTTON_LABEL_I18N_KEYS[buttonLabel]);
    }
    return buttonLabel;
}

export function translateSection(section: any, t: (key: string) => string): any {
    if (typeof section === 'string' && SECTION_I18N_KEYS[section]) {
        return t(SECTION_I18N_KEYS[section]);
    }
    return section;
}
