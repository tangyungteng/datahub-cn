        ...
        (urn: string) => {
            ...
            createExecutionRequestMutation({
                ...
                .then(() => {
                    ...
                    message.success({
                        content: t('ingestion.success_submit_execution'),
                        duration: 3,
                    });
                })
                .catch((e) => {
        ...
        if (focusSourceUrn) {
            ...
            updateIngestionSource({ variables: { urn: focusSourceUrn as string, input } })
                .then(() => {
                    ...
                    message.success({
                        content: t('ingestion.success_update_source'),
                        duration: 3,
                    });
            ...
            createIngestionSource({ variables: { input } })
                .then((result) => {
                    message.loading({ content: t('ingestion.loading_message'), duration: 2 });
                    ...
                    message.success({
                        content: t('ingestion.success_create_source'),
                        duration: 3,
                    });
    ...
    return (
        <>
            ...
            <SourceContainer>
                <HeaderContainer>
                    <StyledTabToolbar>
                        <SearchContainer>
                            <StyledSearchBar
                                placeholder={t('ingestion.search_placeholder')}
                                value={searchInput || ''}
                ...
                {!loading && data?.listIngestionSources?.total === 0 ? (
                    <EmptySources isEmptySearchResult={!!query} />
                ) : (
                    <>
                        <TableContainer>
            ...
            <ConfirmationModal
                ...
                modalTitle={t('ingestion.confirm_execute_title')}
                modalText={t('ingestion.confirm_execute_text')}
                closeButtonText={t('action.cancel')}
                confirmButtonText={t('ingestion.execute_button')}
            />
            <ConfirmationModal
                isOpen={!!sourceUrnToDelete}
                handleConfirm={handleConfirmDelete}
                handleClose={() => setSourceUrnToDelete(null)}
                modalTitle={t('ingestion.confirm_remove_title')}
                modalText={t('ingestion.confirm_remove_text')}
                closeButtonText={t('action.cancel')}
