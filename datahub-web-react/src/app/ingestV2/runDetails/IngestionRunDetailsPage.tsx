...
export default function IngestionRunDetailsPage() {
    ...
    const breadCrumb = (
        <Breadcrumb
            items={[
                {
                    key: 'back',
                    label: fromUrl === tabUrlMap[TabType.RunHistory] ? t('ingestion.run_history_breadcrumb') : t('ingestion.manage_data_sources_breadcrumb'),
    ...
    return (
        <PageLayout title={t('ingestion.run_details')} titlePill={titlePill} rightPanelContent={<AIChat />} topBreadcrumb={breadCrumb}>
            <RunDetailsContent
                urn={urn}
                data={data}
                refetch={refetch}
