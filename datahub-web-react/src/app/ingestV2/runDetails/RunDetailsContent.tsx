...
export default function RunDetailsContent({ urn, data, loading, error, refetch, setTitlePill }: Props) {
        ...
        () =>
            ...
            ResultIcon && (
                <Pill
                    ...
                    label={getExecutionRequestStatusDisplayText(status, t)}
                    color={resultColor}
                    size="md"
                />
            ),
    ...
    return (
        <ContentWrapper>
            {!data && loading && <Message type="loading" content={t('ingestion.loading_run_details')} />}
            {error && message.error(t('ingestion.load_run_details_failed'))}
            <Tabs
                tabs={tabs}
