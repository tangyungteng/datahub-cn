...
function EntityDropdown(props: Props) {
    ...
    const { t } = useTranslation(['entity', 'messages']);
    ...
    const handleUpdateDeprecation = async (deprecatedStatus: boolean) => {
        message.loading({ content: t('messages:loading.updating') });
        try {
            ...
            message.destroy();
            message.success({ content: t('messages:success.deprecation_updated'), duration: 2 });
        } catch (e: unknown) {
            message.destroy();
            if (e instanceof Error) {
                message.error({
                    content: t('messages:error.failed_deprecation', { error: e.message || '' }),
                    duration: 2,
                });
            }
        }
                  ...
                  label: (
                      <MenuItem
                          onClick={() => {
                              ...
                              message.info(t('messages:success.copied_url'), 1.2);
                          }}
                      >
                          <LinkOutlined /> &nbsp; {t('entity:actions.copy_url')}
                      </MenuItem>
                  ),
              }
            : null,
            ...
            ? {
                  ...
                  label: !entityData?.deprecation?.deprecated ? (
                      <MenuItem onClick={() => setIsDeprecationModalVisible(true)}>
                          <ExclamationCircleOutlined /> &nbsp; {t('entity:actions.mark_deprecated')}
                      </MenuItem>
                  ) : (
                      <MenuItem onClick={() => handleUpdateDeprecation(false)}>
                          <ExclamationCircleOutlined /> &nbsp; {t('entity:actions.mark_undeprecated')}
                      </MenuItem>
                  ),
              }
            : null,
        menuItems.has(EntityMenuItems.ADD_TERM)
            ? {
                  ...
                  label: (
                      ...
                      >
                          <MenuItem>
                              <PlusOutlined /> &nbsp;{t('entity:actions.add_term')}
                          </MenuItem>
                      </StyledMenuItem>
                  ),
              }
        ...
        menuItems.has(EntityMenuItems.ADD_TERM_GROUP)
            ? {
                  ...
                  label: (
                      ...
                      >
                          <MenuItem>
                              <FolderAddOutlined /> &nbsp;{t('entity:actions.add_term_group')}
                          </MenuItem>
                      </StyledMenuItem>
                  ),
              }
        ...
        !isDomainMoveHidden && menuItems.has(EntityMenuItems.MOVE)
            ? {
                  ...
                  label: (
                      ...
                      >
                          <MenuItem>
                              <FolderOpenOutlined /> &nbsp;{t('entity:actions.move')}
                          </MenuItem>
                      </StyledMenuItem>
                  ),
              }
                      ...
                      >
                          ...
                          >
                              <MenuItem data-testid="entity-menu-delete-button">
                                  <DeleteOutlined /> &nbsp;{t('entity:actions.delete')}
                              </MenuItem>
                          </Tooltip>
                      </StyledMenuItem>
                  ),
