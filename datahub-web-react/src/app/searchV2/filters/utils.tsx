...
// Maps the static English filter labels (from FIELD_TO_LABEL in fields.tsx, or backend displayName)
// to i18n keys for translation at render time. The raw field value sent to the backend is unchanged.
export const FILTER_LABEL_TO_KEY: Record<string, string> = {
    Type: 'search.filter.type',
    Owner: 'search.filter.owner',
    'Last Modified (In Source)': 'search.filter.last_modified',
    Platform: 'search.filter.platform',
    Tag: 'search.filter.tag',
    'Glossary Term': 'search.filter.glossary_term',
    Domain: 'search.filter.domain',
    Container: 'search.filter.container',
    Entity: 'search.filter.entity',
    'Entity Type': 'search.filter.entity',
    'Sub Type': 'search.filter.sub_type',
    Degree: 'search.filter.degree',
};

// Translates a filter label only for display. Returns the original string when no mapping exists.
export function translateFilterLabel(label: string, t: (key: string) => string): string {
    return FILTER_LABEL_TO_KEY[label] ? t(FILTER_LABEL_TO_KEY[label]) : label;
}
...
export function useFilterDisplayName(filter: FacetMetadata | FilterField, predicateDisplayName?: string) {
    ...
    const label = predicateDisplayName || filter.displayName || filter.field;
    return translateFilterLabel(label, t);
}
